"""Step 71c — STRUCTURE vs LEGIBILITY, the decisive test: INDIRECT observation (single probe).

71/71b both stayed legible because a cycling-basis probe exposed all 3 charge components ~directly each step
-- the readout demanded linear access regardless of the update. 71c removes that: a SINGLE fixed probe e3
observes only ONE projection per step, so the model must reconstruct the 3-D charge from the TIME SERIES of
its rotation (KOBS=12) -- the faithful Phase H Row 2 regime where a generic update could scramble. Nonlinear
readout kept. Same gates. If generic STILL stays legible -> the third leg is robustly refuted (legibility is
bought by amortization + observability, not update structure); if generic scrambles while orthogonal holds
-> third leg confirmed-with-nuance.
...
def gen_batch(B, seed):
    ...
    P = torch.zeros(B, T, 3); P[..., 2] = 1.0  # SINGLE fixed probe e3: only one projection observed per step (indirect)
...
def main():
    mo = train(True, "71c_ortho"); mg = train(False, "71c_generic")
    ...
    (RESULTS / "71c_indirect_obs.json").write_text(json.dumps(res, indent=1))
    ...
    ax[1].legend(fontsize=8); fig.tight_layout(); fig.savefig(RESULTS / "71c_indirect_obs.png", dpi=140)
    print("saved results/71c_indirect_obs.json + .png")
