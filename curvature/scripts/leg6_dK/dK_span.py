...
for chi in (0.075, 0.0375):
    for label, obj, add in (("A unaugmented","A",False), ("A + dK","A",True), ("B + dK","B",True)):
        try:
            ms = [margin(obj, e, 1, chi, add) for e in EPS]
            expo = float(np.polyfit(np.log10(EPS), np.log10(ms), 1)[0])
...
json.dump(out, open(f"{SP}/dK_span.json","w"), indent=2)
print(f"  wrote {SP}/dK_span.json")
