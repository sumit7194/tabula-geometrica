...
EPS=[0.05,0.10,0.20]   # uncensored: every arm >=6x its own floor
print(f"\n  {'arm':>16} " + " ".join(f"{e:>12}" for e in EPS) + f" {'exponent':>9}")
import json
out={}
for label,obj,add in (("A unaug","A",False),("B unaug","B",False),
                      ("A + SHAM","A",True),("B + SHAM","B",True)):
    ms=[margin(obj,e,1,0.075,add) for e in EPS]
    ex=float(np.polyfit(np.log10(EPS),np.log10(ms),1)[0])
    out[label]={"margins":ms,"exponent":ex}
    print(f"  {label:>16} " + " ".join(f"{m:>12.4e}" for m in ms) + f" {ex:>9.3f}", flush=True)
for label,obj in (("A + SHAM floor","A"),("B + SHAM floor","B")):
    f0=margin(obj,0.0,1,0.075,True); out[label]={"floor":f0}
    print(f"  {label:>16} eps=0 -> {f0:.4e}", flush=True)
json.dump(out, open(f"{SP}/sham_ext.json","w"), indent=2)
