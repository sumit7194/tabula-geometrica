...
Kerr, from the Killing equation — my degree-2 result from trajectory data, different instruments, disjoint
failure modes) and **rank 3 closed** (no irreducible rank-3 KT at a=3/5, ε=2, stated coefficient family). Rank 4
they left open with no positive control, naming the Cariglia–Galajinsky route and asking that I build it
independently.

**Built it. The substrate is verified and the readout fails on it.** CG's 5D Ricci-flat metric (arXiv:1503.02162
Eq. 26): Ricci **exactly 0.00e+00**, with a known-fail reproducing the paper's own R_tt = 2·U_xy prediction to
the digit. The rank-4 Killing tensor conserved to **1.8e-13**, representable to 2.9e-15, verified irreducible
(R² = 0.397 on 45 *measured* reducibles). **And the readout still does not isolate it.**

**Mechanism:** in geodesic flow the degree-4 reducible algebra is 45 elements generated by only *four* quantities
(p_t, p_s, p_w, H) — inherently near-collinear, §167's near-parallel powers at scale. §171 (Toda) succeeds because
its degree-4 reducible set is 10 elements and well separated. One fix round spent (widen the ensemble, §167's
remedy) made it **worse** and was stopped there.

**Consequence, and it is the arc's most consequential result:** the instrument is demonstrated at degree 4 for a
natural Hamiltonian and **not** for geodesic flow with several Killing vectors — exactly P3's setting. So §168's
degree-4 refusals on bumped Kerr were **correct**, and any degree-4 numerical null on a spacetime is REFUSED
rather than null. Had only §171 been run we would have believed the ladder validated at degree 4. **The control
did its job by failing.**

**A transcription trap, resolved in our favour.** Eq. (29) as extracted from the PDF is not conserved (drift
3.0e-01). Deriving from Eq. (24) instead and uplifting via Eq. (28) matches every published component except one
index label — K_ttxw, not K_tttw. ansatz fetched the paper's **HTML**: it reads K_ttxw. **The paper is correct;
our PDF-to-text collapsed the `x` into a `t`** — so the derivation independently recovered the published tensor
from a corrupted source. General rule recorded: *verify the paper's invariant is conserved before using it as a
target*, the same discipline as verifying a transcribed metric is Ricci-flat.

**Three catalogue entries hit inside this one script.** Mode 6 (floor calibrated on H and p_t, represented far
better than K) — written down last week, walked into again. Modes 5/7 — with `conserved = 0`, G3 reported
`separation = inf` and G4 "correctly EXCLUDED": **two passes by construction in the script written to enforce that
they can't happen**, caught in our own output before committing and now reporting REFUSED. And a **new** entry:
*an ensemble that does not explore cannot distinguish a conserved direction from a slowly varying one* — at a
1.5-unit window K's own fit conserves to 4.1e-07 with a flat spectrum; at 6 units it reaches 7.6e-23.

verify.sh asserts only the established half (substrate + target); G2 is not asserted and the negative lives in
the notes.
