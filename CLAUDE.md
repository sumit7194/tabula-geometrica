...
- **FULL EMERGENT GRID TORUS (2026-06-24, script 116) — chased down, both gates.**
  ...
  **ISOMETRY** ‖B(θ1)v‖=‖B(θ2)v‖ (the hexagon driver) + reg_u; block-normalize v +
  ...
  Model saved (116_grid_model.pt); --probe-only gate in verify.sh. **Was NOT fast — 714.9 s / 7.09 GB and 806.9 s / 6.80 GB across two measured runs, ~30% of the suite; the word "fast" had never been measured (silent_nulls 24). FIXED 2026-08-22: the probe now reads saved persistence diagrams (`116_dgms.npz`) instead of re-running 32 Ripser calls — 1.70 s / 0.32 GB, H2 still asserted in full, plus a weights fingerprint so stale diagrams fail rather than pass.** The
...
- **CORNER STUDY (quantum/vestigium, scripts 180-186, 2026-09-01) — CLOSED with an honest null on the headline.**
  ...
  **G2** a(60)≈0.0204, a(120)≈0.00287, spreads 0.73%/1.12% + full pairwise matrix. **G3 ✓ decisively** — area
  ...
  shows nothing. 180 + 186 in verify.sh. **silent_nulls → 68** (55: an artifact that does not record the MODE it was produced in cannot be checked
  ...
  because they labelled it inferred rather than verified. 48: a correction that INHERITS the unexamined premise of what it corrects is the same
  mistake at higher confidence — instanced by 45→46 in this repo, an hour apart. 47: a plateau in a nuisance
  parameter is evidence ONLY when the same sweep is shown to MOVE on a known-fail control — checked against
  §178's flat degree ladder, which survives because L1 descends 89,109× (47/48 relayed by TheBridge from a
  third workspace, unverified by either of us; the 46 instance is first-hand). 46: a path-based identity check reads the LAUNCHER's argv, not the process — a
